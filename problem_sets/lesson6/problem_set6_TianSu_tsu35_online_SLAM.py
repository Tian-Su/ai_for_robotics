...
class matrix:
    ...
    def CholeskyInverse(self):
        ...
        for j in reversed(range(self.dimx)):
            tjj = self.value[j][j]
            S = sum([self.value[j][k] * res.value[j][k] for k in
                     range(j + 1, self.dimx)])
...
class robot:
    ...
    def sense(self):
        ...
        for i in range(self.num_landmarks):
            dx = self.landmarks[i][
                     0] - self.x + self.rand() * self.measurement_noise
            dy = self.landmarks[i][
                     1] - self.y + self.rand() * self.measurement_noise
            if self.measurement_range < 0.0 or abs(dx) + abs(
                    dy) <= self.measurement_range:
                Z.append([i, dx, dy])
        return Z
...
def online_slam(data, N, num_landmarks, motion_noise, measurement_noise):
    ...
    for k in range(len(data)):
        ...
        for i in range(len(measurement)):
            ...
            # index of the landmark coordinate
            m = 2 * (1 + measurement[i][0])

            # update the based on the measurement
            for j in range(2):
                Omega.value[j][j] += 1.0 / measurement_noise
                Omega.value[m + j][m + j] += 1.0 / measurement_noise
                Omega.value[j][m + j] += -1.0 / measurement_noise
                Omega.value[m + j][j] += -1.0 / measurement_noise
                Xi.value[j][0] += -measurement[i][1 + j] / measurement_noise
                Xi.value[m + j][0] += measurement[i][1 + j] / measurement_noise

        # expand for the new position
        ...
        # update the information maxtrix/vector based on the robot motion
        for i in range(4):
            Omega.value[i][i] += 1.0 / motion_noise

        for i in range(2):
            Omega.value[i][i + 2] += -1.0 / motion_noise
            Omega.value[i + 2][i] += -1.0 / motion_noise
            Xi.value[i][0] += -motion[i] / motion_noise
            Xi.value[i + 2][0] += motion[i] / motion_noise

        matrix_A = Omega.take([0, 1], range(2, dim + 2))
        matrix_B = Omega.take([0, 1], [0, 1])
        matrix_C = Xi.take([0, 1], [0])

        Omega_p = Omega.take(range(2, dim + 2), range(2, dim + 2))
        Omega = Omega_p - matrix_A.transpose() * matrix_B.inverse() * matrix_A

        Xi_p = Xi.take(range(2, dim + 2), [0])
        Xi = Xi_p - matrix_A.transpose() * matrix_B.inverse() * matrix_C
...
def print_result(N, num_landmarks, result):
    ...
    for i in range(N):
        print '    [' + ', '.join(
            '%.3f' % x for x in result.value[2 * i]) + ', ' \
              + ', '.join('%.3f' % x for x in result.value[2 * i + 1]) + ']'
    print
    print 'Estimated Landmarks:'
    for i in range(num_landmarks):
        print '    [' + ', '.join(
            '%.3f' % x for x in result.value[2 * (N + i)]) + ', ' \
              + ', '.join(
...
testdata1 = [[[[1, 21.796713239511305, 25.32184135169971],
               [2, 15.067410969755826, -27.599928007267906]],
              [16.4522379034509, -11.372065246394495]],
             [[[1, 6.1286996178786755, 35.70844618389858],
               [2, -0.7470113490937167, -17.709326161950294]],
              [16.4522379034509, -11.372065246394495]],
             [[[0, 16.305692184072235, -11.72765549112342],
               [2, -17.49244296888888, -5.371360408288514]],
              [16.4522379034509, -11.372065246394495]],
             [[[0, -0.6443452578030207, -2.542378369361001],
               [2, -32.17857547483552, 6.778675958806988]],
...
answer_omega1 = matrix([[0.36603773584905663, 0.0, -0.169811320754717, 0.0,
                         -0.011320754716981133, 0.0, -0.1811320754716981, 0.0],
                        [0.0, 0.36603773584905663, 0.0, -0.169811320754717, 0.0,
                         -0.011320754716981133, 0.0, -0.1811320754716981],
                        [-0.169811320754717, 0.0, 0.6509433962264151, 0.0,
                         -0.05660377358490567, 0.0, -0.40566037735849064, 0.0],
                        [0.0, -0.169811320754717, 0.0, 0.6509433962264151, 0.0,
                         -0.05660377358490567, 0.0, -0.40566037735849064],
                        [-0.011320754716981133, 0.0, -0.05660377358490567, 0.0,
                         0.6962264150943396, 0.0, -0.360377358490566, 0.0],
                        [0.0, -0.011320754716981133, 0.0, -0.05660377358490567,
                         0.0, 0.6962264150943396, 0.0, -0.360377358490566],
                        [-0.1811320754716981, 0.0, -0.4056603773584906, 0.0,
                         -0.360377358490566, 0.0, 1.2339622641509433, 0.0],
                        [0.0, -0.1811320754716981, 0.0, -0.4056603773584906,
                         0.0, -0.360377358490566, 0.0, 1.2339622641509433]])
...
testdata2 = [[[[0, 12.637647070797396, 17.45189715769647],
               [1, 10.432982633935133, -25.49437383412288]],
              [17.232472057089492, 10.150955955063045]],
             [[[0, -4.104607680013634, 11.41471295488775],
               [1, -2.6421937245699176, -30.500310738397154]],
              [17.232472057089492, 10.150955955063045]],
             [[[0, -27.157759429499166, -1.9907376178358271],
               [1, -23.19841267128686, -43.2248146183254]],
              [-17.10510363812527, 10.364141523975523]],
             [[[0, -2.7880265859173763, -16.41914969572965],
               [1, -3.6771540967943794, -54.29943770172535]],
              [-17.10510363812527, 10.364141523975523]],
             [[[0, 10.844236516370763, -27.19190207903398],
               [1, 14.728670653019343, -63.53743222490458]],
...
answer_omega2 = matrix([[0.22871751620895048, 0.0, -0.11351536555795691, 0.0,
                         -0.11351536555795691, 0.0],
                        [0.0, 0.22871751620895048, 0.0, -0.11351536555795691,
                         0.0, -0.11351536555795691],
                        [-0.11351536555795691, 0.0, 0.7867205207948973, 0.0,
                         -0.46327947920510265, 0.0],
                        [0.0, -0.11351536555795691, 0.0, 0.7867205207948973,
                         0.0, -0.46327947920510265],
                        [-0.11351536555795691, 0.0, -0.46327947920510265, 0.0,
                         0.7867205207948973, 0.0],
                        [0.0, -0.11351536555795691, 0.0, -0.46327947920510265,
                         0.0, 0.7867205207948973]])
