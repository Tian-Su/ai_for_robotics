...
class robot:
    ...
    def cte(self, radius):
        ...
        if radius < self.x < 3 * radius:
            ...
            if self.x < 2 * radius:
                cte = ((self.x - center1[1]) ** 2 + (
                    self.y - center1[0]) ** 2) ** 0.5 - radius
            else:
                cte = ((self.x - center2[1]) ** 2 + (
                    self.y - center2[0]) ** 2) ** 0.5 - radius
        return cte
...
def run(params, radius, printflag=False):
    ...
    crosstrack_error = myrobot.cte(
        radius)  # You need to define the cte function!
