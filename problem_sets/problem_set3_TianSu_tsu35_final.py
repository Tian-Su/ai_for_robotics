...
class robot:
    ...
    def move(self, motion):  # Do not change the name of this function
        ...
        # ADD CODE HERE
        new_rob = robot()
        new_rob.length = self.length
        new_rob.bearing_noise = self.bearing_noise
        new_rob.steering_noise = self.steering_noise
        new_rob.distance_noise = self.distance_noise

        d = random.gauss(motion[1], self.distance_noise)
        alpha = random.gauss(motion[0], self.steering_noise)
        beta = d / new_rob.length * tan(alpha)
        if abs(beta) < 0.001:
            new_rob.x = self.x + d * cos(self.orientation)
            new_rob.y = self.y + d * sin(self.orientation)
            new_rob.orientation = (self.orientation + beta) % (2 * pi)
        else:
            R = d / beta
            cx = self.x - sin(self.orientation) * R
            cy = self.y + cos(self.orientation) * R
            new_rob.orientation = (self.orientation + beta) % (2 * pi)
            new_rob.x = cx + sin(new_rob.orientation) * R
            new_rob.y = cy - cos(new_rob.orientation) * R
        return new_rob  # make sure your move function returns an instance
    ...
    def sense(self, noise_flag=1):  # do not change the name of this function
        Z = []

        # ENTER CODE HERE
        # landmarks  = [[0.0, 100.0], [0.0, 0.0], [100.0, 0.0], [100.0, 100.0]]
        if noise_flag:
            b1 = (atan2(landmarks[0][0] - self.y, landmarks[0][1] - self.x) - self.orientation) % (2 * pi)
            b2 = (atan2(landmarks[1][0] - self.y, landmarks[1][1] - self.x) - self.orientation) % (2 * pi)
            b3 = (atan2(landmarks[2][0] - self.y, landmarks[2][1] - self.x) - self.orientation) % (2 * pi)
            b4 = (atan2(landmarks[3][0] - self.y, landmarks[3][1] - self.x) - self.orientation) % (2 * pi)
        # HINT: You will probably need to use the function atan2()
        else:
            b1 = (atan2(landmarks[0][0] - self.y, landmarks[0][1] - self.x) - self.orientation + self.bearing_noise) % (
            2 * pi)
            b2 = (atan2(landmarks[1][0] - self.y, landmarks[1][1] - self.x) - self.orientation + self.bearing_noise) % (
            2 * pi)
            b3 = (atan2(landmarks[2][0] - self.y, landmarks[2][1] - self.x) - self.orientation + self.bearing_noise) % (
            2 * pi)
            b4 = (atan2(landmarks[3][0] - self.y, landmarks[3][1] - self.x) - self.orientation + self.bearing_noise) % (
            2 * pi)
