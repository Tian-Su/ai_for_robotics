...
def execute_student_plan(warehouse, todo, max_distance, max_steering):
    student_planner = partB.DeliveryPlanner(warehouse, todo, max_distance,
                                            max_steering)
...
class State:
    ...
    def _set_initial_state_from(self, warehouse, todo):
        ...
        self.warehouse_limits['segments'].append(
            [(0.0, self.warehouse_limits['min_y'] + self.ROBOT_RADIUS),
             (self.warehouse_limits['max_x'],
              self.warehouse_limits['min_y'] + self.ROBOT_RADIUS)])
        # East segment
        self.warehouse_limits['segments'].append(
            [(self.warehouse_limits['max_x'] - self.ROBOT_RADIUS,
              self.warehouse_limits['min_y']),
        ...
        self.robot = robot.Robot(x=self.dropzone['ctr_x'],
                                 y=self.dropzone['ctr_y'], bearing=0.0,
    ...
    def update_according_to(self, action):
        ...
        if action_type == 'move':
            ...
            raise Exception(
                'improperly formatted action: {}'.format(''.join(action)))
    ...
    def _attempt_move(self, steering, distance):
        ...
        try:
            ...
            if is_legal_move:
                ...
                self._increase_total_cost_by(
                    self.ILLEGAL_MOVE_PENALTY * distance)
                self.robot.move(steering, clear_distance)

        except ValueError:
            raise Exception(
                'improperly formatted move destination: {} {}'.format(row, col))
    ...
    def _attempt_lift(self, box_id):
        ...
        try:
            ...
            box_position = (
            self.boxes[box_id]['ctr_x'], self.boxes[box_id]['ctr_y'])
    ...
    def _attempt_down(self, x, y):
        ...
        try:
            ...
            raise Exception(
                'improperly formatted down destination: {} {}'.format(row, col))
    ...
    def _is_box_within_warehouse(self, coordinates):
        x, y = coordinates
        return self.BOX_SIZE < x < (
        self.warehouse_limits['max_x'] - self.BOX_SIZE) and \
               (self.warehouse_limits[
                    'min_y'] + self.BOX_SIZE) < y < -self.BOX_SIZE

        # Check if the box within the dropzone

    def _is_box_within_dropzone(self, coordinates):
        x, y = coordinates
        return (self.dropzone['min_x'] + self.BOX_SIZE) < x < (
        self.dropzone['max_x'] - self.BOX_SIZE) and \
               (self.dropzone['min_y'] + self.BOX_SIZE) < y < (
               self.dropzone['max_y'] - self.BOX_SIZE)
    ...
    def _is_open_for_box(self, coordinates):
        ...
        for o in self.obstacles:
            if not self._is_box_outside_obstacle(coordinates, o):
                print '*** Could not set down - obstacle {} in the way ***'.format(
                    0)
                return False
    ...
    def _is_traversable(self, destination, distance, steering):
        ...
        for o in self.obstacles:
            # do a coarse check to see if the center of the obstacle
            # is too far away to be concerned with
            dst = self._distance_point_to_line_segment((o['ctr_x'], o['ctr_y']),
                                                       t0, t1)
        ...
        dst, intercept_point = self._check_intersection(t0, t1,
                                                        self.warehouse_limits)
        if dst < min_distance_to_intercept:
            min_distance_to_intercept = dst
            min_intercept_point = intercept_point
        ...
        if min_distance_to_intercept < chk_distance:
            ...
            print "*** Robot crashed at {p[0]:6.2f} {p[1]:6.2f} ***".format(
    ...
    def _check_intersection(self, t0, t1, obstacle):
        ...
        for s in obstacle['segments']:
            dst, intercept_point = self._linesegment_intersection(t0, t1, s[0],
                                                                  s[1])
            if dst < min_distance_to_intercept:
                min_distance_to_intercept = dst
                min_intercept_point = intercept_point
    ...
    def _corner_intersection(self, t0, t1, corner):
        ...
        if disc >= 0.:
            dp2 = dm2 = 1.e6
            if corner['min_x'] <= xp <= corner['max_x'] and corner[
                'min_y'] <= yp <= corner['max_y']:
                dp2 = (xp - t0[0]) ** 2 + (yp - t0[1]) ** 2

            if corner['min_x'] <= xm <= corner['max_x'] and corner[
                'min_y'] <= ym <= corner['max_y']:
                dm2 = (xm - t0[0]) ** 2 + (ym - t0[1]) ** 2

            if dp2 < dm2:
                # make sure the intersection pointn is actually on the trajectory segment
                if self._distance_point_to_line_segment((xp, yp), t0,
                                                        t1) < 1.e-6:
                    dst = math.sqrt(dp2)
                    intercept_point = (xp, yp)
            else:
                if self._distance_point_to_line_segment((xm, ym), t0,
                                                        t1) < 1.e-6:
                    dst = math.sqrt(dm2)
    ...
    def _dilate_obstacle_for_robot(self, obstacle):
        ...
        obstacle['segments'] = []
        # West segnent
        obstacle['segments'].append(
            [(obstacle['min_x'] - self.ROBOT_RADIUS, obstacle['max_y']), \
             (obstacle['min_x'] - self.ROBOT_RADIUS, obstacle['min_y'])])
        # South segment
        obstacle['segments'].append(
            [(obstacle['min_x'], obstacle['min_y'] - self.ROBOT_RADIUS), \
             (obstacle['max_x'], obstacle['min_y'] - self.ROBOT_RADIUS)])
        # East segment
        obstacle['segments'].append(
            [(obstacle['max_x'] + self.ROBOT_RADIUS, obstacle['min_y']), \
             (obstacle['max_x'] + self.ROBOT_RADIUS, obstacle['max_y'])])
        # North segment
        obstacle['segments'].append(
            [(obstacle['max_x'], obstacle['max_y'] + self.ROBOT_RADIUS), \
    ...
    def print_to_console(self):
        print ''
        print 'Robot state:'
        print '\t x = {:6.2f}, y = {:6.2f}, hdg = {:6.2f}'.format(self.robot.x,
                                                                  self.robot.y,
                                                                  self.robot.bearing * 180. / PI)
        print 'Box state:'
        for b in self.boxes:
            box = self.boxes[b]
            print '\tbox id {}, x = {:6.2f}, y = {:6.2f}'.format(b,
                                                                 box['ctr_x'],
                                                                 box['ctr_y'])
...
class PartBTestCase(unittest.TestCase):
    ...
    def run_test_with_params(self, k):
        ...
        return params['min_cost'] / cost * float(len(delivered)) / float(
            len(params['todo']))
